<?php
/*
Plugin Name: Related Posts with thumbnails of Aitu 爱徒-智能社会化推荐相关文章插件
Plugin URI: http://http://wordpress.org/extend/plugins/aitu-releated-posts-with-thumbnails/
Author: 51aitu Team
Version: 1.1.0.0
Author URI: http://tuijian.51aitu.com
Description: Related Posts with thumbnails,利用关键字提取和分词、客户行为分析技术，智能匹配相关文章，并以图片形式展示。
 
Copyright 2010 51aitu.com (email: hatcloud@gmail.com)
*/

if (!class_exists('aituRelatedPosts')) {
    class aituRelatedPosts {
        /*
         * !!! You must do the two things before you try to do local test with this plugin:
         * 1. turn on AITU_DEBUG mode
         * 2. change AITU_SERVER to "http://{computer-name}:8080"
         *    {computer-name} is the name of computer which running a local test 51aitu web server.
         */
        const AITU_DEBUG = false;
        //const AITU_SERVER = 'http://localhost:8080/apps/tuijian';
        const AITU_SERVER = 'http://tuijian.51aitu.com';
        
        const VERSION = '1.0.5.5';
        const ADMIN_OPTION_NAME = 'aitu_admin_options';
        const PLUGIN_PATH = '/wp-content/plugins/aitu-related-posts';
        ...
        const NOTIFY_MESSAGE_CONTAINER = 'aitu_notify_message_container';
        
        const VERIFICATION_CODE = 'aitu_verification_code';
        ...
        function addPluginActionLinks($links, $file) {
            if ($file == plugin_basename(__FILE__)) {
                $links[] = '<a href="options-general.php?page=aitu-related-posts.php">' . __('Settings') . '</a>';
            }
            return $links;
        }
...
AITU_HOOK;
        ...
        function echoVerificationMeta() {
            ...
            if ($code) {
                echo "<meta name='51aituVerification' content='$code' />\n";
            }
            else
                echo "<meta name='no51aituVerification' content='$code' />\n";
        }
        ...
        private function createAituScript() {
            ...
            $script = '';
            if (self::AITU_DEBUG) {
                $script = "<script type='text/javascript'>var 51aituDebugServer = '$server';</script>";
            }
...
<p style="margin:0;padding:0;height:1px;overflow:hidden;">
    <script type="text/javascript"><!--
        var aituSitePrefix = "$this->sitePrefix";
        var aituEnableCustomPos = $enableCustomPos;
        var aituParams = "$queryParams";
    //--></script><script type="text/javascript" src="$server/index.php/tuijian/relatedjs"></script><a href="$server/index.php/tuijian/related" style="border:0;"><img src="$server/images/aitu.png" alt="爱徒相关文章插件，快速提升流量" style="border:0;padding:0;margin:0;" /></a>
</p>
AITU_SCRIPT;
        ...
        function printAdminPage() {
            ...
            if (array_key_exists('add_verification_meta', $_POST)) {
                ...
                if ($this->isHookFuncInTemplate('header.php', 'wp_head')) {
                    ...
                    if ($code) {
                        $OptionsUpdatedMessage[] = new Aitu_Notify_Message('已成功添加认证代码，现在请回到<a href="http://tuijian.51aitu.com/index.php" target="_blank">爱徒网站管理中心</a>，点击“验证”按钮即可完成博客认证。');
                    }
                } else {
        ...
        function checkFooterScriptSupportedByTheme($theme) {
            if ($this->getAdminOption(self::SCRIPT_IN_FOOTER) && !$this->isScriptInFooterSupported()) {
                $this->pushNotifyMessage('此主题不支持将<strong>爱徒相关文章脚本</strong>置于文档末尾, 可能会导致相关文章无法显示, 建议关闭此功能.', 'error');
            }
        }
        ...
        function doActivation() {
            ...
            $this->pushNotifyMessage('<strong>爱徒相关文章插件: </strong>1 如果您的博客装有缓存(cache)插件, 请在首次启用插件时清空缓存. 2 如果是第一次启用，请在启用后到<strong><a href="http://tuijian.51aitu.com/index.php" target="_blank">爱徒网站管理中心</a></strong>注册并提交网站获取验证码 3 请到后台设置-爱徒相关文章，设置验证码 4 回到<strong><a href="http://tuijian.51aitu.com/index.php" target="_blank">爱徒网站管理中心</a></strong>点击“开始验证”即可。此时爱徒爬虫会立刻去获取贵站的文章信息, 这时切记不要停掉插件, 以免影响文章的相关性.装完可来<strong><a href="http://tuijian.51aitu.com/index.php" target="_blank">爱徒网站管理中心</a></strong>查看收录状况.');
        }
    ...
    function classConflictException() {
        echo '<div class="error"><p><strong>插件冲突。</strong>您的博客正在运行一个与“爱徒相关文章插件”定义了相同类名的插件，只有在关闭冲突插件以后“爱徒相关文章插件”才能正常启用。</p></div>';
    }
